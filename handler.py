...
def _ler_pid_comfyui():
    """PID do ComfyUI gravado pelo start.sh, ou None (patch krea2-comfy-api)."""
    try:
        return int(open(COMFY_PID_FILE).read().strip())
    except Exception:
        return None
...
def handler(job):
    ...
    # --- PATCH krea2-comfy-api (11/09/2026) -------------------------------------------------------
    # Sonda: mata o ComfyUI de proposito para o teste da trava abaixo. So responde se o token bater
    # com SONDA_TOKEN do template; sem a variavel, o campo e ignorado.
    # SONDA_SEM_REFRESH=1 desliga o refresh_worker: e a mutacao que deixa tests/trava_worker_morto.py
    # vermelha. Nunca defina isso num template de producao.
    refresh = os.environ.get("SONDA_SEM_REFRESH") != "1"
    token = os.environ.get("SONDA_TOKEN")
    if token and job_input.get("sonda_matar") == token:
        pid = _ler_pid_comfyui()
        if pid:
            os.kill(pid, 9)
            time.sleep(1)
        return {"error": "sonda: ComfyUI morto de proposito", "refresh_worker": refresh, "sonda_pid": pid}
    ...
    if not check_server(
        ...
        # Sem refresh_worker o worker fica "pronto" com o ComfyUI morto e todo pedido que cai nele
        # falha em 0,3 s (visto em 11/09: 6 falhas seguidas no endpoint de video). Com refresh_worker
        # o RunPod descarta este worker depois da resposta e o proximo pedido nasce num worker novo.
        return {
    ...
    try:
        ...
        # PATCH krea2-comfy-api: ComfyUI morreu NO MEIO da geracao (visto em 11/09: "ComfyUI HTTP
        # unreachable during websocket reconnect"). Mesmo tratamento: descarta o worker.
        return {"error": f"WebSocket communication error: {e}", "refresh_worker": refresh and _is_comfyui_process_alive() is False}
    except requests.RequestException as e:
        print(f"worker-comfyui - HTTP Request Error: {e}")
        print(traceback.format_exc())
        return {"error": f"HTTP communication error with ComfyUI: {e}", "refresh_worker": refresh and _is_comfyui_process_alive() is False}
